...
- `docs/skills/problem-framing.md` — Schon's problem setting, Dorst's frame innovation, six reframing moves, abstraction laddering, Ackoff's dissolution, design fixation, wicked problems
- `docs/skills/strategic-thinking.md` — Inversion and pre-mortem protocol, opportunity cost, second-order thinking, upstream analysis, systems leverage points, strategic recommendation framework
...
Use **AskUserQuestion** to ask: "What plan, decision, or direction should I challenge?"
...
- Use **Glob** and **Read** to find relevant files, architectural decisions, or in-progress work
- Use **Grep** to search for related patterns, dependencies, or prior discussions

**Web research (if available):**
Use **WebSearch** to understand the current state of the topic:
- `"<topic> best practices" OR "<topic> architecture"`
- `"<topic> case study" OR "<topic> experience report"`
- `"<topic> failure" OR "<topic> postmortem" OR "<topic> lessons learned"`
...
"How attached are you to the current direction? This calibrates how hard I push."

Options:
- **Open to anything** — "Challenge everything — I want the best path, not the current one"
- **Directionally committed** — "I believe the direction is right but want to stress-test the approach"
- **Validating** — "I'm mostly looking for confirmation and edge cases"
...
**Web research:**
Use **WebSearch**:
- `"<core problem> reframing" OR "<core problem> alternative approach"`
- `"<core problem> root cause" OR "why <core problem> fails"`
...
2. Use **WebSearch** to validate alternatives:
   - `"<alternative approach> vs <current approach>"`
   - `"instead of <current approach>" OR "<current approach> alternatives"`
...
**Web research:**
Use **WebSearch**:
- `"<topic> upstream" OR "<topic> root cause" OR "<topic> prevention"`
- `"<topic> systems thinking" OR "<topic> leverage point"`
