...
- `docs/skills/fishbone-analysis.md` — Ishikawa technique, standard category sets, decomposition method, evidence grading, ASCII diagram format, prioritization
- `docs/skills/root-cause-analysis.md` — Root cause categories, 5 Whys technique, hypothesis testing (for follow-up tracing)
...
Use **AskUserQuestion** to ask: "What's the problem you want to map causes for?"
Options:
- "Recurring bug" — Same area keeps breaking in different ways
- "Production incident" — Something failed and you want to understand all contributing factors
- "Systemic issue" — A pattern of problems across the system
- "Pre-mortem" — Mapping what could go wrong before it does
...
If the default set isn't a good fit, use **AskUserQuestion** to confirm the category set with the user.
...
- Use **Read** and **Grep** to examine code, configuration, and infrastructure related to the problem
- Use **Bash** with `git log` and `git diff` to check recent changes
- Use **WebSearch** to research known issues in relevant libraries or infrastructure
- Use **WebFetch** on authoritative sources (official docs, issue trackers)
- Look for evidence that confirms, suspects, or rules out each candidate cause
