...
- `docs/skills/dag-analysis.md` — DAG validation, antipatterns, decomposition strategies
- `docs/skills/topological-sort.md` — Kahn's algorithm, critical path method, wave scheduling
- `docs/skills/mermaid-flowcharts.md` — Diagram conventions, styling, node limits
...
Use **AskUserQuestion** to ask: "What would you like to map?"
Options:
- "Task dependencies" — Build a dependency graph and execution order
- "Process flow" — Diagram a workflow or pipeline
...
2. **Codebase scan:**
   Use **Grep** to find TODOs, FIXMEs, and task markers:
   - Pattern: `TODO|FIXME|HACK|XXX`
   - Also check for task tracking files, issue trackers, or backlog files
...
2. If the description is vague:
   - Use **WebSearch** to research standard patterns (e.g., "CI/CD pipeline stages", "code review workflow")
   - Use **WebFetch** on the most relevant result
