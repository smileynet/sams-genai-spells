...
- `docs/skills/program-comprehension.md` — Top-down vs. bottom-up reading strategies, expert code reading patterns, mental model building, scope limiting
- `docs/skills/architecture-patterns.md` — Common architecture patterns to recognize, signal detection, Mermaid diagram conventions, module boundary heuristics
...
Options:
- "A specific module or directory" — Explore one part of the codebase in depth
- "A feature or behavior" — Trace how a specific feature is implemented end-to-end
- "The overall architecture" — Map the high-level structure and module relationships
- "A specific question" — Answer a targeted question about how the code works
...
- Use **Glob** to find relevant files by naming patterns and directory structure
- Use **Grep** to find imports, exports, class/function definitions, and cross-references
- Use **Read** to check for README, docs, or comments that describe architecture
- Identify the module boundary — what's inside vs. outside the scope
- Count files, estimate complexity, note languages and frameworks
...
If scope is too broad, use **AskUserQuestion** to ask: "This area has [N] files. Which part should I focus on?"
Offer the 2-4 most distinct sub-areas as options based on directory structure or functional grouping.
...
1. **Identify entry points** — Use **Grep** to find exports, route handlers, main functions, public APIs, or CLI entry points
2. **Follow the call chain** — Use **Read** to trace from entry points through internal modules. Follow function calls, not file order.
3. **Track data flow** — Where does data enter? How is it transformed? Where does it go? Follow the data, not the control flow.
4. **Identify key abstractions** — Classes, interfaces, types, and patterns that organize the system. Use **Grep** to find type definitions, base classes, or interfaces.
5. **Note design decisions** — Patterns chosen (MVC, event-driven, layered), trade-offs made, conventions followed
6. **Use web research if needed** — Use **WebSearch** to understand unfamiliar frameworks or patterns the code depends on. Research the framework, not the codebase.
